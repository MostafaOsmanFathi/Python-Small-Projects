...
 
def dfs(s, vis, subset):
    vis[s] = True
    for e in v[s]:
        if e in subset and not vis[e]:
            dfs(e, vis, subset)
 
def ver(a):
    for i in range(len(a)):
        for j in range(i + 1, len(a)):
            if (a[i], a[j]) in dislike or (a[j], a[i]) in dislike:
                return False
    ...
    vis = {x: False for x in a}
    dfs(a[0], vis, set(a))
    if all(vis[i] for i in a):
        return True
...
for mask in range(1 << n):
    ...
    for i in range(n):
        if mask & (1 << i):
            subset.append(i + 1)
    if len(subset) > 0 and ver(subset):
